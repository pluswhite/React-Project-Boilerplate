...
describe('(Component) Counter', () => {
  ...
  test('renders with an <h2> that includes Counter label.', () => {
    expect(_wrapper.find('h2').text()).to.match(/Counter:/)
  })

  test('renders {props.counter} at the end of the sample counter <h2>.', () => {
  ...
  describe('Increment', () => {
    ...
    test('is a primary button', () => {
      expect(_button.hasClass('btn btn-primary')).to.be.true()
    })

    test('Calls props.increment when clicked', () => {
  ...
  describe('Double Async Button', () => {
    ...
    test('is a secondary button', () => {
      expect(_button.hasClass('btn btn-secondary')).to.be.true()
    })

    test('Calls props.doubleAsync when clicked', () => {
      _spies.dispatch.should.have.not.been.called()
